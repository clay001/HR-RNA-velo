...
class VelocytoLoom:
    ...
    n_pca_dims: int = None,                                                                                                                                                                 
    def knn_imputation_VI(self, k: int = None, metric: str = "euclidean", diag: float = 1,
                       maximum: bool = False, size_norm: bool = True,
                       balanced: bool = False, b_sight: int = None, b_maxl: int = None,
                       group_constraint: Union[str, np.ndarray] = None, n_jobs: int = 8) -> None:
        """Performs k-nn smoothing of the data matrix
        knn will be performed in AutoEncoder space (`ls`)
        ...
        if balanced:
            if group_constraint is not None:
                if isinstance(group_constraint, str) and group_constraint == "clusters":
                    constraint = np.array(self.cluster_ix)
                bknn = BalancedKNN(k=k, sight_k=b_sight, maxl=b_maxl, metric=metric, constraint=constraint,
